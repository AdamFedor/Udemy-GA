...
d3.select("input")
    .on("input", function() {
      ...
      d3.selectAll("rect")
        ...
        // add this to get started
        .transition()
        .duration(2000)
        .ease(d3.easeLinear)
        .delay((d,i) => i * 250)
        .on('start', function (d, i) {
          if (i === 0) { // listening to the first rectangle
          d3.select('.title')
            .text('updating to ' + year + ' data...');
          }
        })
        .on('end', function (d, i) {
          if (i === 0) { // listening to the last rectangle
          d3.select('.title')
            .text('Birth Data in ' + year);
          }
        })
        .on('interrupt', function () {
          console.log('New year added, no longer updating to previous year')
        })
          .attr("height", function(d) {
            return height - yScale(d.births);
          })
